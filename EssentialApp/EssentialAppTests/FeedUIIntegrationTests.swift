...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadFeedActions_requestFeedFromLoader() {
        ...
        sut.simulateUserInitiatedReload()
        XCTAssertEqual(loader.loadFeedCallCount, 3, "Expected yet another loading request once view is loaded")
    }
    ...
    func test_feedImageView_loadsImageURLWhenVisiable() {
        ...
        XCTAssertEqual(loader.loadedImageURLs, [], "Expected no images URL requests until views become visiable")
    ...
    func test_feedImageView_cancelsImageLoadingWhenNotVisiableAnymore() {
        ...
        sut.simulateFeedImageViewNotVisible(at: 0)
        XCTAssertEqual(loader.cancelledImageURLs, [image0.url], "Expected one cancelled image URL requests once first image is not visiable anymore")
    ...
    func test_feedImageViewRetryAction_retriesImageLoad() {
        ...
        let view0 = sut.simulateFeedImageViewVisible(at: 0)
        let view1 = sut.simulateFeedImageViewVisible(at: 1)
        XCTAssertEqual(loader.loadedImageURLs, [image0.url, image1.url], "Expectd two image URL request for the two visible views")
        
        loader.completeImageLoadingWithError(at: 0)
        loader.completeImageLoadingWithError(at: 1)
        XCTAssertEqual(loader.loadedImageURLs, [image0.url, image1.url], "Expectd only two image URL requests before retry action")
    ...
    func test_feedImageView_reloadsImageURLWhenBecomingVisiableAgain() {
        let image0 = makeImage(url: URL(string: "http://url-0.com")!)
        let image1 = makeImage(url: URL(string: "http://url-1.com")!)
        let (sut, loader) = makeSUT()
        ...
        XCTAssertEqual(loader.loadedImageURLs, [image0.url, image0.url, image1.url, image1.url], "Expected two image URL request after second view becomes visible again")
    }
    
    func test_feedImageView_configuresViewCorrectlyWhenCellBecomingVisiableAgain() {
        let (sut, loader) = makeSUT()
    ...
    func test_feedImageView_doesNotShowDataFromPreviousRequestWhenCellIsReused() {
        let (sut, loader) = makeSUT()
        sut.simulateAppearance()
    ...
    func test_loadMoreActions_requestMoreFromLoader() {
        ...
        sut.simulateLoadMoreFeedAction()
        XCTAssertEqual(loader.loadMoreCallCount, 2, "Expected request after load more completion with more pages")
    ...
    func test_feedImageView_doesNotLoadImageAgainUntilPreviousRequestCompletes() {
        ...
        sut.simulateFeedImageViewNearVisible(at: 0)
        XCTAssertEqual(loader.loadedImageURLs, [image.url], "Expected first request when near visiable")
...
//    test_feedImageView_doesNotLoadImageAgainUntilPreviousRequestCompletes
    ...
    private func makeSUT(selection: @escaping (FeedImage) -> Void = { _ in }, file: StaticString = #file, line: UInt = #line) -> (sut: ListViewController, loader: LoaderSpy) {
        ...
        let sut = FeedUIComposer.feedComposeWith(feedLoader: {
            loader.loadPublisher()
        }, imageLoader: {
            loader.loadImageDataPublisher(from: $0)
        }, selection: selection)
