...
class CommentsUIIntegrationTests: XCTestCase {
        ...
        autoreleasepool {
            sut = CommentsUIComposer.commentsComposedWith(commentsLoader: {
                PassthroughSubject<[ImageComment], Error>()
                    .handleEvents(receiveSubscription: nil, receiveCancel: {
                        cancelCallCount += 1
                    }).eraseToAnyPublisher()
            })
    ...
    func assertThat(_ sut: ListViewController, isRendering comments: [ImageComment], file: StaticString = #filePath, line: UInt = #line) {
        ...
        viewModel.comments.enumerated().forEach { index, comment in
            XCTAssertEqual(sut.commentMessage(at: index), comment.message, "mesage at index \(index)", file: file, line: line)
            XCTAssertEqual(sut.commentData(at: index), comment.date, "date at index \(index)", file: file, line: line)
            XCTAssertEqual(sut.commentUsername(at: index), comment.username, "date at index \(index)", file: file, line: line)
        }
    }
